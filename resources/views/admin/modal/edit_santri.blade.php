<div class="panel panel-white">
    ...
    <h4 class="panel-title">Tambah Data {{$profiles->user->name}}</h4>
    </div>
    <div class="panel-body">
        <form action="{{route('put-daftar-santri')}}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}                        
           
        <input type="hidden" name="id" value="{{$profiles->id}}">
        <div class="form-group">
            <label for="name">NIS</label>
        <input type="text" class="form-control" value="{{$profiles->nis}}" name="nis" id="nis" placeholder="nis">
        <div class="form-group">
        ...
        <select name="level" id="level" class="form-control">
            ...
            <option value="pengurus" @if($profiles->user->level=='pengurus') selected @endif  >Pengurus</option>
            <option value="santri" @if($profiles->user->level=='santri') selected @endif  >Santri</option>
            <option value="admin" @if($profiles->user->level=='admin') selected @endif >Admin</option>
        </select>
        </div>
        <div class="form-group">
        ...
        <div class="modal-footer mt-5">
            ...
            <a href="{{route('resetpassword',['id'=>$profiles->user->id])}}" class="btn btn-danger">Reset Password</a>
